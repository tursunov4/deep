...
const category: Ref<components["schemas"]["List_subcategories"] | undefined> = ref()
...
<template>
  <div class="flex flex-col items-center desktop:mt-[155px] mt-[120px] desktop:mb-[45px] mb-[50px]">
    <div v-if="category" class="flex flex-col items-center gap-[14px] desktop:gap-[30px] mx-auto my-auto">
      <h1
          class="title text-center desktop:text-[80px] uppercase desktop:leading-[80px] text-[40px] leading-[40px]">
        {{ App.language == 'ru' ? category.category.rus_name : category.category.eng_name }}</h1>
      <div
  ...
  <div>
    <!--breadcrumbs-->
    <ul class="flex px-4 gap-5 self-start mt-0 text-[12px] desktop:ml-[13px] ml-[4px]">
      <li @click="router.push('/')" class="cursor-pointer">{{ t('breadcrumbs.home') }}</li>
      <li>{{ t('breadcrumbs.categories') }}</li>
      <li v-if="category">
        {{ App.language == 'ru' ? category.category.rus_name.toUpperCase() : category.category.eng_name.toUpperCase() }}
      </li>
    </ul>
    <!--breadcrumbs end-->

    <!--list of subcategories-->
    <div v-if="category" :class="[category.subcategories.length < 2 ? 'border-t-0' : 'border-t']"
      ...
      <div @click="router.push('/sub-category/' + subcategory.eng_name.toUpperCase())"
        ...
        <img :src="HOST + subcategory.banner" class="object-cover w-full aspect-[71/34]"/>
        <div class="flex justify-between desktop:mt-[21px] desktop:mb-[-4px] mt-[7px] mb-[15px]">
          <div class="title text-[18px]">
            {{ App.language == 'ru' ? subcategory.rus_name.toUpperCase() : subcategory.eng_name.toUpperCase() }}
          </div>
