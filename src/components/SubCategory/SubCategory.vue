...
const preferredCollection: Ref<components["schemas"]["ShowColInSubc"] | undefined> = ref()

const subcategory: Ref<components["schemas"]["List-collections"] | undefined> = ref()
...
async function fetchPages(previewFilters ?: Filters) {
  ...
  if (firstSearch) {
    body = {
      "subcategory_eng_name": router.currentRoute.value.params.id as string,
      "prefered_collection": filters.value ? undefined : parseInt(route.query.collection as string),
      "lowest_cost": filters.value ? filters.value.range[0] : 0,
      "highest_cost": filters.value ? filters.value.range[1] : 99999999,
      "order_by": "date" as SortFilters,
      "colours": route.query?.color ? [route.query?.color as string] : undefined,
      "collections": filters.value?.collections ? (filters.value.collections).map(str => +str) : undefined,
    ...
    body = {
      "subcategory_eng_name": router.currentRoute.value.params.id as string,
      "prefered_collection": previewFilters ? undefined : parseInt(route.query.collection as string),
      "lowest_cost": previewFilters ? previewFilters.range[0] : 0,
      "highest_cost": previewFilters ? previewFilters.range[1] : 99999999,
      "order_by": "date" as SortFilters,
      "colours": previewFilters?.colors ? (previewFilters.colors).map(str => +str) : undefined,
      "collections": previewFilters?.collections ? (previewFilters.collections).map(str => +str) : undefined,
    ...
    body = {
      ...
      "highest_cost": filters.value ? filters.value.range[1] : 99999999,
      "order_by": "date" as SortFilters,
      "colours": filters.value?.colors ? (filters.value.colors).map(str => +str) : undefined,
      "collections": filters.value?.collections ? (filters.value.collections).map(str => +str) : undefined,
...
<template>
  <div class="w-full">
    <div class="flex flex-col items-center desktop:mt-[155px] mt-[120px] desktop:mb-[45px] mb-[50px]">
      <div class="flex flex-col items-center gap-[14px] desktop:gap-[30px] mx-auto my-auto">
        <h1
          ...
          {{ App.language == 'ru' ? subcategory?.subcategory?.rus_name : subcategory?.subcategory?.eng_name }}</h1>
        <div
    ...
    <ul class="flex desktop:gap-5  main-p-sides gap-[30px] mt-[60px] desktop:mt-0 self-start">
      ...
      <li v-if="subcategory">{{
          App.language == 'ru' ? subcategory.subcategory.rus_name.toUpperCase() : subcategory.subcategory.eng_name.toUpperCase()
        }}
      </li>
    </ul>
    <div>
      <div class="desktop:hidden mx-[20px] mt-6">
        <div class="border-y border-black w-full h-full">
          <keep-alive>
            <Filters @clear-filter="productsAmount = undefined" @hideFilters="closeFilter"
                     @preview="newFilters => fetchPages(newFilters)"
                     @set-filters="newFilters => filters = newFilters " :count="productsAmount" v-if="showFilters"
                     class="desktop:hidden"/>
          </keep-alive>
          <div v-if="!showFilters" @click="openFilters"
            ...
            <div v-if="productsAmount != undefined"
      ...
      <div class="flex main-p-sides mt-6">
        <div
          ...
          <div v-if="preferredCollection && currentPage == 1"
               class="w-full aspect-[2/1] desktop:aspect-auto col-span-2">
            <div
                class="relative w-full h-full text-white darker flex flex-col p-4">
              <img :src="preferredCollection.banner" class="object-cover top-0 left-0 -z-10 w-full h-full absolute"/>
              <h1 class="title mx-auto my-auto">{{
                  App.language == 'ru' ? preferredCollection.rus_name.toUpperCase() : preferredCollection.eng_name.toUpperCase()
                }}</h1>
              <div class="place-self-start">
                {{ App.language == 'ru' ? preferredCollection.rus_description : preferredCollection.eng_description }}
              </div>
            </div>
          </div>
          <div v-if="preferredCollection" v-for="(product, index) in preferredCollection.abstracts"
               class="cursor-pointer w-full  flex flex-col p-4"
               :class="[{'desktop:col-span-3 desktop:aspect-[2/1]': preferredCollection.abstracts.length - 1 == index && preferredCollection.abstracts.length % 4 == 2}, {'desktop:col-span-2 desktop:aspect-[3/1]': preferredCollection.abstracts.length - 1 == index  && preferredCollection.abstracts.length % 4 == 3}]">
            <ProductCard :product-group="product"/>
          </div>
          <template v-for="(product, index) in subcategory?.objects_on_page">
            <div v-if="product.collection_type" :class="currentPage == 1 ? 'col-span-2 aspect-[2/1]' : 'col-span-1'"
                 class="w-full desktop:aspect-auto ">
              <div
                  class="relative w-full h-full text-white darker flex flex-col p-4">
                <img :src="product?.banner" class="object-cover top-0 left-0 -z-10 w-full h-full absolute"/>
                <h1 class="title mx-auto uppercase my-auto">{{
                    App.language == 'ru' ? product.rus_name : product.eng_name
                  }}</h1>
                <div class="place-self-start">
                  {{ App.language == 'ru' ? product.rus_description : product.eng_description }}
                </div>
              </div>
            </div>
            <div v-else-if="currentPage == 1"
                 :class="index == subcategory?.objects_on_page.length - 1 && index % 2 == 1 ? 'col-span-2 !border-r-0 aspect-[350/240] desktop:col-span-1 desktop:!border-r px-0 desktop:p-[30px] aspect-[aspect-[385/440]:' : ''"
            ...
            <div v-else
                 :class="[index == subcategory?.objects_on_page.length - 1 && index % 2 == 0 ? 'col-span-2 !border-r-0 aspect-[350/240] desktop:col-span-1 desktop:!border-r px-0 desktop:p-[30px] aspect-[aspect-[385/440]:' : '', index == subcategory?.objects_on_page.length - 1 ? 'desktop:border-r' : '']"
                 class="cursor-pointer odd:border-r odd:desktop:border-r-0  odd:pl-0 even:pr-0 even:desktop:p-[30px] odd:desktop:p-[30px] border-black border-b aspect-[175/240] desktop:aspect-[385/440] w-full flex flex-col desktop:p-[30px] p-[20px]">
              <ProductCard :product-group="product"/>
            </div>
          </template>
          <div
              class="overflow-auto col-start-4 row-start-1 row-span-full self-start desktop:block hidden bg-black-general text-white content-start">
            <Filters @preview="filters => fetchPages(filters)" @set-filters="newFilters => filters = newFilters"
                     :count="productsAmount"/>
          </div>
        </div>
      </div>
      <div class="main-p-sides mt-[80px]">
        <vue-awesome-paginate class="font-TT desktop:mt-auto w-full"
