...
<template>
  <div v-if="videoFile" class="w-full h-full relative bg-black text-white">
    <div class="title text-[40px] hidden desktop:absolute top-9 left-6">{{t('video.name')}}</div>
    <!--    <div class="title absolute desktop:top-[30px] top-[20px] left-[20px] desktop:left-[30px]">{{t('video.name')}}</div>-->
    <video playsInline v-if="videoFile.file" poster="../../assets/poster.png" @click="playVideo" :src="videoFile.file" id="videoPlayer"
           class="w-full h-full flex items-center object-cover justify-center">
    </video>
    <img @click="playVideo" src="../../assets/Play.svg" :class="{'hidden': playing}"
    ...
    <div class="flex desktop:absolute gap-6">
      <div class="flex absolute bottom-[30px] left-[20px] desktop:left-[30px] gap-6">
        <button v-if="about" @click="router.push('/about')" class="border pt-1 whitespace-nowrap cursor-pointer border-white hover:bg-white hover:text-black font-bold font-TT desktop:text-[18px] text-[16px] desktop:leading-[18px] leading-[16px] desktop:w-[240px] w-[120px] desktop:h-[50px] h-[40px]">
          {{t('video.about')}}
        </button>
      </div>
    </div>
