...
<template>
  <div class="w-full flex flex-wrap desktop:flex-row min-h-screen relative">
    <div
      ...
      <RouterLink to="/doors" class="w-full h-full main-p gap-4 text-white flex flex-col justify-end">
        <div class="title text-[40px] leading-[40px] desktop:leading-[60px]">{{ t('main.doors') }}</div>
        <div class="text-[16px]">{{ t('main.left_desc') }}</div>
      </RouterLink>
    </div>
    <div class="desktop:w-1/2 w-full flex flex-col">
      <div v-if="mainSubs[0]" @click="router.push('/sub-category/' + mainSubs[0].eng_name)"
        ...
        <div class="title text-[40px] leading-[40px] desktop:leading-[60px]">
          {{ App.language == 'ru' ? mainSubs[0].rus_name.toUpperCase() : mainSubs[0].eng_name.toUpperCase() }}
        </div>
        <div class="text-[16px]">{{ t('main.right_top_desc') }}</div>
      </div>
      <div v-if="mainSubs[1]" @click="router.push('/sub-category/' + mainSubs[1].eng_name)"
        ...
        <img :src="mainSubs[1].banner"
             class="absolute -z-10 top-0 left-0 w-full h-full object-cover brightness-75 desktop:brightness-100"/>
        <div class="title text-[40px] leading-[40px] desktop:leading-[60px]">
          {{ App.language == 'ru' ? mainSubs[1].rus_name.toUpperCase() : mainSubs[1].eng_name.toUpperCase() }}
        </div>
