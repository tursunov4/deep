...
const collection: Ref<components["schemas"]["ObjsCollection"] | undefined> = ref()
  ...
  <div v-if="collection" class="w-full block-gap main-p-sides">
    <div class="flex mb-[30px]">
      <div class="flex items-center w-1/2">
        <div class="title hidden desktop:block text-[40px]">{{ t('featured_collection.featured_collection') }}</div>
      </div>
      <div class="w-1/2 flex justify-end items-end gap-4 text-black text-base">
        <RouterLink to="/collections">
          <a class="title text-base">{{ t('featured_collection.all_collections') }}</a></RouterLink>
        <!--        <a @click="router.push('/collections')" class="title cursor-pointer text-base">{{t('featured_collection.all_collections')}}</a>-->
      </div>
    </div>
    <div class="w-full flex desktop:flex-row flex-col border-black desktop:border-r ">
      <div
        ...
        <img :src="collection.featured_banner ?? collection.banner" class="w-full h-full object-cover absolute top-0 left-0 -z-10"/>
        <div class="title text-[40px]">{{ collection.rus_name }}</div>
        <div class="desktop:static absolute bottom-3 left-3">Scandinavian mood. Stone, Wood & Metal</div>
      </div>
      <div v-if="collection.products" class="desktop:w-1/2 w-full flex mt-4 desktop:mt-0 flex-wrap">
        <div class="w-full flex desktop:aspect-[2/1] aspect-[350/240]">
          ...
          <div class="cursor-pointer w-1/2 h-full  pr-0 desktop:p-[30px] p-[20px] border-y border-black">
