...
async function distributeImages(
  imageUrls: components["schemas"]["PreviewInspiration"][],
  columnCount: number,
  forMobile = false
) {
  const columns: components["schemas"]["PreviewInspiration"][][] = Array.from(
    { length: columnCount },
    () => []
  );
  const columnHeights = new Array(columnCount).fill(0);
  const filteredImages = imageUrls.filter(
    (url) => url.id.toString() !== router.currentRoute.value.params.id
  );
...
<template>
  <div
    v-if="isZoomed && isDesktop"
    @click="isZoomed = false"
    class="absolute z-[10] pt-[140px] top-0 h-full w-full bg-black/40"
  >
    <div class="relative">
      <img :src="inspiration?.image" class="w-full" />
      <button
        @click="isZoomed = false"
        class="absolute top-[30px] z-40 right-[30px] close-zoom border-white border aspect-square w-[88px] h-[88px] font-light"
      ></button>
    </div>
  </div>
  <div
  ...
  >
    ...
    <div
      v-if="nearItems.previous && isDesktop"
      @click="
        router.push({
          name: 'inspiration-page',
          params: { id: nearItems.previous?.id },
        })
      "
    ...
    >
      <img
        ...
        class="w-[10px] h-[20px] object-contain rotate-180"
      />
    </div>
    <div
      v-if="nearItems.next && isDesktop"
      @click="
        router.push({
          name: 'inspiration-page',
          params: { id: nearItems.next?.id },
        })
      "
    ...
    <div class="grow flex max-h-full relative max-w-full overflow-hidden">
      <div
        v-if="inspiration?.products_in.length"
      ...
      <div class="grow flex justify-center items-center max-w-full max-h-full">
        <template v-for="item in inspiration?.products_in">
          <div
            v-if="item.colored_product && (isDesktop || showItems)"
            :style="{
              left: `${
                offset.left -
                (isDesktop ? 44 : 20) +
                offset.width * (item.x_coordinate / 100)
              }px`,
              top: `${
                offset.top -
                (isDesktop ? 44 : 20) +
                offset.height * (item.y_coordinate / 100)
              }px`,
            }"
          ...
          >
            ...
            <div
              :class="[
                item.x_coordinate > 75
                  ? 'right-[0px] pr-[150px]'
                  : 'left-[0px] pl-[150px]',
                { 'bottom-0': item.y_coordinate > 80 },
                { 'top-0': item.y_coordinate < 20 },
              ]"
              class="w-[620px] h-[250px] hover:block group-hover:desktop:block hidden absolute"
            >
              <div
                class="w-full h-full relative z-10 bg-white p-[30px] flex gap-[30px]"
              >
                <img
                ...
                <div class="flex w-full flex-col gap-[20px]">
                  ...
                  <h2 class="font-TT font-bold text-[18px] uppercase">
                    {{
                      App.language == "ru"
                        ? item.abstract_product_preview.rus_name
                        : item.abstract_product_preview.eng_name
                    }}
                  </h2>
                  <div>
                    {{
                      App.language == "ru"
                        ? item.colored_product.rub_cost + "₽"
                        : item.colored_product.euro_cost + "€"
                    }}
                  </div>
                  <router-link
                    class="mt-auto"
                    :to="'/product/' + item.abstract_product_preview.id"
                  >
                    <button
  ...
  <div v-if="inspiration?.support_products_for?.length">
    ...
    <ProductScroller
      v-if="inspiration?.support_products_for"
  ...
  <div>
    ...
    <div
      class="desktop:mx-[30px] mx-[20px] flex justify-center desktop:my-[80px] mt-[40px] mb-[60px]"
    >
...
.close-zoom:hover:before,
.close-zoom:hover:after {
  position: absolute;
  content: " ";
  height: 30px;
  border-radius: 2px;
...
.crosshair::after {
  ...
  height: 1px;
  background-color: rgba(152, 127, 127, 0); /* Adjust color as needed */
  backdrop-filter: invert(100%);
  opacity: 0;
...
.crosshair.shown:before,
.crosshair.shown:after {
  opacity: 1;
}
