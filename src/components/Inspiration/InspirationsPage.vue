...
async function getInspiration() {
  {
    const {data} = await POST('/api/inspiration/page/{pag_num}/', {
      ...
      body: {
        authors: filters.value?.authors?.length ? filters.value?.authors : undefined,
        ...
        building: filters.value?.building?.length ? filters.value?.building : undefined,
...
<template>
  <div class="w-full">
    <div class="flex flex-col items-center desktop:mt-[155px] mt-[120px] desktop:mb-[100px] mb-[60px]">
    ...
    <ul class="flex desktop:gap-5  main-p-sides gap-[30px] mt-[60px] self-start">
    ...
    <div>
      <div class="desktop:hidden border-black  mt-6">
        <div class="w-full">
          <div class="desktop:hidden h-[2px] border-b border-black mx-[20px] my-[20px]"/>
          <div ref="filterContainer" id="mobileFilter" class=""/>
          <div v-if="showFilters" class="desktop:hidden h-[2px] border-b border-black mx-[20px] my-[20px]"/>
        </div>
      </div>
      <div class="flex gap-[20px] min-h-screen main-p-sides desktop:mt-6">
        <div
          ...
          <div v-for="column in (isDesktop ? distributedImages : distributedImagesMobile)" class="">
        ...
        <teleport v-if="filterContainer" :disabled="!filterContainer || isDesktop" :to="filterContainer">
            <div
                 class="desktop:w-1/4 w-full">
              <InspirationFilters @toggle-filter="showFilters = !showFilters"
