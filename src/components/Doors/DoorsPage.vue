...
<template>
  <div class="desktop:block hidden">
    <div class="flex flex-col items-center desktop:mt-[155px] mt-[120px]  mb-[50px]">
      <div class="flex flex-col uppercase items-center gap-[14px] desktop:gap-[30px] mx-auto my-auto">
        <h1
            class="title text-center  desktop:text-[80px] desktop:leading-[78px] text-[40px] leading-[40px]">
          {{
            App.language === 'ru' ? info?.ru_name : info?.eng_name
          }}</h1>
        <div class="desktop:text-[18px] min-h-[27px] text-[10px] font-TT font-bold text-center">
  ...
  <div
    ...
    <MainCaurousel v-if="info?.cover_images.length" :images="info?.cover_images ? [...info?.cover_images] : []"/>
  </div>
  <div class="desktop:hidden w-2/3 px-[20px]">
    <div class="flex flex-col desktop:mt-[155px] mt-[43px] desktop:mb-[100px] mb-[20px]">
      <div class="flex uppercase flex-col gap-[10px] desktop:gap-[10px] desktop:mx-auto my-auto">
        <h1
            class="title desktop:text-[80px] pr-[40px] desktop:leading-[80px] text-[40px] leading-[40px]">
          {{
            App.language === 'ru' ? info?.ru_name : info?.eng_name
          }}</h1>
  ...
  <div class="desktop:mx-[30px] desktop:py-[30px] py-[20px] border-black border-y mx-[20px]">
    <div class="desktop:-mx-[30px] -mx-[20px]">
      <DoorsCarousel/>
    </div>
  </div>
  <div
      class="desktop:mb-[30px] mb-[20px] desktop:text-[36px] desktop:mt-[80px] mt-[60px] text-[40px] font-TT font-bold leading-[36px] desktop:mx-[30px] mx-[20px] uppercase">
    {{ App.language === 'ru' ? info?.combination_title_name_ru : info?.combination_title_name_eng }}
  </div>
  <div class="overflow-visible">
    <TypesCarousel/>
  </div>
  <div :style="{'background-color': `${info?.hight_background}`}"
    ...
    <div class="desktop:w-1/2 w-2/3 h-full">
      <h2 class="font-TT text-[40px] leading-[40px] desktop:mb-[40px] mb-[20px] font-bold uppercase">
        {{ App.language === 'ru' ? info?.hight_title_name_ru : info?.hight_title_name_eng }}
      </h2>
      <div v-html="App.language === 'ru' ? info?.hight_description_ru : info?.hight_description_eng"
  ...
  <div class="w-full flex justify-center aspect-[390/160] items-center desktop:aspect-[1600/320] relative">
    <div class="absolute z-10 top-0 left-0 h-full w-full bg-black/20"/>
    <img :src="info?.collections_background_image ?? ''" class="absolute z-0 top-0 left-0 w-full h-full object-cover"/>
    <button @click="router.push('/catalogs/#doors')"
  ...
  <div class="desktop:px-[30px] desktop:block hidden px-[20px] uppercase">
    <div class="font-TT text-[40px] mt-[80px] mb-[30px] leading-10 uppercase font-bold">
      {{ t('doors_page.finishing_materials') }}
    </div>
    <div class="w-full flex desktop:flex-nowrap flex-wrap border-black border-t border-l mt-6">
      <div v-for="cover in [...covers].slice(0, 4)"
           class="cursor-pointer desktop:w-1/4 w-1/2 aspect-square border-black border-r border-b desktop:p-[30px] p-2">
        <div
            class="w-full mb-4 aspect-square"><img class="w-full h-full object-cover" :src="cover?.image"/></div>
        <div class="w-full flex justify-between">
          <div>{{ App.language === 'ru' ? cover?.rus_name : cover?.eng_name }}</div>
          <div>{{ t('doors_page.explore') }}</div>
        </div>
      </div>
    </div>
    <div ref="coversElement" :style="{'height': '0'}"
         class="w-full section flex desktop:flex-nowrap flex-wrap border-black border-l">
      <div v-for="cover in [...covers].slice(4)"
           class="cursor-pointer desktop:w-1/4 w-1/2 aspect-square border-black border-r border-b desktop:p-[30px] p-2">
        <div
            class="w-full mb-4 aspect-square"><img class="w-full h-full object-cover" :src="cover?.image"/></div>
        <div class="w-full flex justify-between">
    ...
    <div class="flex justify-center">
      <button
          @click="coversCollapsed ? expandSection(coversElement) : collapseSection(coversElement); coversCollapsed = !coversCollapsed"
  ...
  <div
    ...
    {{ App.language === 'ru' ? info?.handle_title_name_ru : info?.handle_title_name_eng }}
  </div>
  <div class="border-black desktop:border border-b desktop:mx-[30px]">
    <div v-for="handle in [...handles].slice(0, 1)" class="desktop:grid justify-items-stretch content-stretch grid-cols-2">
      ...
      <div
        ...
        <div :class="[handlesCollapsed  ? 'border-b-0' : 'border-b']" class="font-TT font-normal border-black desktop:border-0 uppercase text-[25px] desktop:text-[40px] desktop:pb-0 pb-[20px] leading-7 desktop:leading-10 text-black/40">
          {{ App.language == 'ru' ? handle?.ru_description : handle?.eng_description }}
        </div>
      </div>
    </div>
    <div ref="handlesElement" class="section" :style="{'height': '0'}">
      <div v-for="(handle, index) in [...handles].slice(1)" class="desktop:grid content-stretch justify-items-stretch grid-cols-2">
        <div :style="{'background-image': `url(${handle.image})`}" class="bg-no-repeat desktop:w-auto desktop:aspect-auto w-full aspect-[416/214] bg-cover bg-center"/>
        <div
            class="desktop:px-[30px] px-[20px] py-[30px] break-words">
          <h3 class="font-TT font-bold uppercase text-[25px] desktop:text-[40px] mb-[40px]">
            {{ App.language == 'ru' ? handle?.name_ru : handle?.name_eng }}</h3>
          <div :class="[index === [...handles].slice(1).length - 1 ? 'border-b-0' : 'border-b']" class="font-TT font-normal uppercase text-[25px] border-black desktop:text-[40px] desktop:leading-10 leading-7 desktop:pb-0 pb-[20px] text-black/40">
            {{ App.language == 'ru' ? handle?.ru_description : handle?.eng_description }}
          </div>
        </div>
      </div>
  ...
  <div class="desktop:px-[30px] px-0 mb-[80px] uppercase">
    <div class="font-TT leading-10 px-[20px] desktop:px-0 text-[40px] mt-[80px] mb-[30px] font-bold">
      {{ App.language === 'ru' ? info?.lock_title_name_ru : info?.lock_title_name_eng }}
    </div>
    <div v-for="(lock, index) in locks">
      <div :class="[index === 0 ? 'desktop:mt-0' : 'desktop:mt-[120px]']" class="w-full desktop:mb-[80px]">
        <div
          ...
          <div class="flex-1 desktop:px-[30px] px-[20px] py-[30px] border-b border-r border-black break-words">
            <h3 class="font-TT font-bold desktop:text-[40px] desktop:leading-[40px] text-[25px] desktop:mb-[40px] mb-[20px]">
              {{ App.language == 'ru' ? lock?.name_ru : lock?.name_eng }}</h3>
            <div class="font-TT font-normal desktop:text-[40px] text-[25px] desktop:leading-10 leading-7 opacity-40">
              {{ App.language == 'ru' ? lock?.ru_description : lock?.eng_description }}
            </div>
          </div>
        </div>
        <div
          ...
          <div v-for="step in lock.step_images"
            ...
            <div class="w-full relative">
              <img :src="step.image" class="w-full aspect-[136/145] block m-auto">
            </div>
            <div class="flex justify-between text-[10px] mt-2">
              <div class="">{{ App.language == 'ru' ? step?.name_ru : step?.name_eng }}</div>
            </div>
          </div>
        </div>
      </div>
